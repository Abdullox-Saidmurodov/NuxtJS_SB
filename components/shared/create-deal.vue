...
import { DB_ID, COLLECTION_DEALS } from '~/constants/index'
import { DATABASE, UNIQUE_ID } from '~/libs/appwrite'
import { useAuthStore } from '~/store/auth.store'
import { EnumStatus, type ICreateDeals } from '~/types/index'
...
<template>
    <UPopover :popper="{ placement: 'right' }">
        <UBytton variant="ghost" class="opacity-75 hover:opacity-100 mx-auto mt-3" color="blue">
            ...
            <div class="p-4 w-80 dark:bg-gray-900 bg-gray-100">
                <UForm :validate="validate" :state="state" class="space-y-4" @submit="onSubmit">
                    ...
                    <UFormGroup label="Description" name="description">
                        <!-- <UInput v-model="state.description" color="blue" block size="lg" /> -->
                        <UTextarea v-model="state.description" color="blue" block size="lg" />
                    </UFormGroup>

                    <UButton type="submit" color="blue" class="w-full" block size="lg" :disabled="isPending">
                        <template v-if="isPending"><Icon name="svg-spinners:3-dots-scale" class="w-5 h-5" /></template>
